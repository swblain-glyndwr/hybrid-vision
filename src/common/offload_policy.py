...
class RLDecisionPolicy:
    ...
    def update(self,
               ...
               action: str,
               reward: float,
               next_metrics: Iterable[float]) -> None:
        key = self._state_key(metrics)
        next_key = self._state_key(next_metrics)
        a = self.actions.index(action)
        best_next = np.max(self.q[next_key])
        self.q[key][a] += self.alpha * (reward + self.gamma * best_next - self.q[key][a])
...
class QuickDecisionPolicy:
    ...
    def __init__(self,
                 ...
                 bandwidth_threshold: float | None = None) -> None:
